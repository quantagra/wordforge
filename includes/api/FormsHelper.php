...
class FormsHelper
{
    ...
    public function create_post($form_settings, $form_data, $current_post_id, $dynamic_post_id, $form_files)
    {
        ...
        if ($post_thumbnail) {
            ...
            $post_thumbnail = $this->handle_file($post_thumbnail, $form_settings, $form_files, "id");
        }
    ...
    {
        global $wpdb;
        $form_fields = $this->get_form_fields_from_ids($form_settings, $form_data, $post_id, $form_id);
        ...
        foreach ($form_fields as $field) {
            $field_value = $this->get_form_field_by_id($field['id'], $form_data, null, null, $form_files, true, true);
    ...
    private function get_form_fields_from_ids($form_settings, $form_data, $post_id, $form_id)
    {
        $form_fields = array();
                ...
                if (isset($form_files) && !empty($form_files)) {
                    ...
                    foreach ($form_files as $file) {
                        ...
                        if (strpos($file['field'], '_') !== false) {
                            $file['field'] = explode('_', $file['field'])[0];
                        }
        ...
        if ($uploaded_file && isset($form_files) && count($form_files)) {
            ...
            $uploaded_file = array_filter($form_files, function ($item) use ($uploaded_file) {
                ...
                if (strpos($item['field'], '_') !== false) {
                    $field_id = explode('_', $item['field']);
                    return $uploaded_file === $field_id[0];
                }
